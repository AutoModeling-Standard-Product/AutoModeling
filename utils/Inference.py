...
#             w_tr = re_weight_by_org(y_tr, tr_orgidx, 0.5, broadcast_with_tar, balanced_badrate)
        ...
        if 'stopping_rounds' in self.param.keys():
            ...
            X_tr, X_val, y_tr, y_val, tr_orgidx, val_orgidx = self.inference_split_data(data, True)
    ...
    ## 基于建模样本单一机构做oos评估
    def refit_cvoos_(self, _, org, param):
        tr_idxs = set(self.X_tr.index)
        oos_idx = set(self.tr_orgidx.get(org))
        X_tr_, y_tr_ = self.X_tr.loc[list(tr_idxs-oos_idx), ], self.y_tr.loc[list(tr_idxs-oos_idx), ]
        
        if param.get('balanced_badrate') is not None:
            w_tr_ = self.w_tr.loc[list(tr_idxs-oos_idx), ]
        else:
            w_tr_ = pd.Series(np.ones(X_tr_.shape[0]), index=X_tr_.index)
        train_set = lgb.Dataset(X_tr_, y_tr_, weight=w_tr_)
        X_oos, y_oos = self.X_tr.loc[list(oos_idx), ], self.y_tr.loc[list(oos_idx), ]
    ...
    def get_cvoos_result(self, data):
        assert 'stopping_rounds' not in self.param.keys(), "参数中存在stopping_rounds, 无法生成cv oos"
        broadcast_with_tar = self.param.get('broadcast_with_tar')
        balanced_badrate = self.param.get("balanced_badrate")
...
#             self.w_tr = re_weight_by_org(self.y_tr, self.tr_orgidx, 0.5, broadcast_with_tar, balanced_badrate)
    ...
    def generate_report(self, data, oos_data):
        ...
        sns.boxplot(y='new_org', x=self.child_score, data=data, ax=ax1)
        fig1.savefig(self.store_pth+"/train logs/trend/子分箱线图.jpg")
        plt.close()
        
        fig2, ax2 = plt.subplots(figsize=(10, 6))
        sns.boxplot(y='new_org', x=self.score_name, data=data, ax=ax2)
