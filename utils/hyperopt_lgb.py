...
class HyperOptLGB(object):
    ...
    def __init__(self, **kwargs):
        ...
        self.X_tr, self.X_val, self.y_tr, self.y_val, self.tr_orgidx, self.val_orgidx = self.split_data(kwargs.get('data'))
        self.trails = Trials()
    ...
    def weighted_metric(self, weights):
        ...
        def _weighted_metric(pred_, data):
            ...
            auc, auc_w= roc_auc_score(y, pred), roc_auc_score(y, pred, sample_weight=w)
    ...
    def extract_evalresult(self, records):
        ...
        for org in records.org.unique():
            ...
            simpler_records.append({
                'org': org, 'tr_auc':tmp_record.train.auc_wo[-1], 'tr_auc_w':tmp_record.train.auc_w[-1],
                ...
                'val_auc':tmp_record.val.auc_wo[-1], 'val_auc_w':tmp_record.val.auc_w[-1],
                ...
                'oos_auc': tmp_record.oos.auc_wo[-1], 'oos_ks':tmp_record.oos.ks[-1],
    ...
    def train_epoch_(self, _, org, param):
...
#         w_tr_, w_val_ = weight.loc[list(tr_idxs-set(tr_idx)), ], weight.loc[list(val_idxs-set(val_idx)), ]
        ...
        w_tr_ = pd.Series(np.where(y_tr_==1, balanced_badrate, 1), index=y_tr_.index)
        w_val_ = pd.Series(np.where(y_val_==1, balanced_badrate, 1), index=y_val_.index)
        ...
        if self.record_train_process == False:
            ...
            tmp0 = pd.DataFrame({'train':[[auc_w], [auc], [ks_w], [ks], [lift5], [lift10]]}, index=['auc_w', 'auc_wo', 'ks_w', 'ks', '5%lift', '10%lift'])
            auc_w, auc, ks_w, ks, lift5, lift10 = self.single_weighted_metric(model, X_val_, y_val_, w_val_)
            tmp1 = pd.DataFrame({'val':[[auc_w], [auc], [ks_w], [ks], [lift5], [lift10]]}, index=['auc_w', 'auc_wo', 'ks_w', 'ks', '5%lift', '10%lift'])
            auc_w, auc, ks_w, ks, lift5, lift10 = self.single_weighted_metric(model, X_oos, y_oos, pd.Series(np.ones(len(y_oos))))
            tmp2 = pd.DataFrame({'oos':[[auc_w], [auc], [ks_w], [ks], [lift5], [lift10]]}, index=['auc_w', 'auc_wo', 'ks_w', 'ks', '5%lift', '10%lift'])
    ...
    def objective(self, param):
        ...
        with open("res.txt", 'a') as f:
            json.dump({'loss': loss, 'param': param}, f)
